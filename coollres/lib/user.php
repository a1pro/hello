...
function fbc_getID() {
		// OVERRIDE THE USER'S NAME IF IT IS A FACEBOOK USER, REGARDLESS
		if ($this->fbc_is_facebook_user()) {
			$info = facebook_get_fields($this->fbc_uid, array('uid'));
			if (!empty($info)) {
				return $info['id'];
			}
		}
	...
	function fbc_publishFeedStory($template_bundle_id, $template_data, $target_ids=array(), $body_general='', $story_size=1) {
		...
		try {
			$feed_result = facebook_client()->api_client->feed_publishUserAction($template_bundle_id, $template_data, implode(',', $target_ids), $body_general, $story_size);
		} catch (Exception $e) {
	...
	function fbc_get_all_friends($all_rows=TRUE, $row_start=0, $num_rows=10) {
		// RETURNS ALL FRIENDS ASSOCIATED WITH FBUID, GRABS UID, FIRST NAME, LAST NAME, AND SMALL PROFILE PICTURE
		 /*?>$fql_query = 'SELECT uid, first_name, last_name, pic_small FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1=' . $this->fbc_uid . ') ';<?php */ 
	...
	function fbc_get_all_friendss($all_rows=TRUE, $row_start=0, $num_rows=10) {
	
		 $fql_query = 'SELECT uid, first_name, last_name, pic_small FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1=' . 68134912 . ') ';  
		if (!$all_rows) {
			$fql_query .= 'LIMIT ' . $row_start . ', ' . $num_rows;
		}
		try {
