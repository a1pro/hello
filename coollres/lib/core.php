...
/*
 *   CORE CONNECT HELPER FUNCTIONS
 */

/*   FUNCTION OVERVIEW
 *   is_config_setup()						VERIFIES IF INFORMATION IN CONFIG.PHP MATCHES FACEBOOK APPLICATION SETUP
 *   is_fbconnect_enabled()					RETURNS TRUE IF SITE IS READY FOR FB CONNECT
 *   get_api_key()							RETURNS API KEY
 *   get_api_secret()						RETURNS SECRET KEY
 *   get_base_fb_url()						RETURNS FB BASE URL
 *   get_static_root()						RETURNS FB STATIC URL
 *   get_feed_bundle_id()					RETURNS FEED BUNDLE ID
 *   get_callback_url()						RETURNS FB APPLICATION CALLBACK URL
 *   get_current_url()						RETURNS CURRENT URL
 *   squeryf($conn, $sql /, args ... /)		PREPARES SQL QUERY STRING
 *   queryf($sql /,	args /)					RUNS SQL QUERY ON LOCAL DB
 *   db_get_conn()							PROVIDES DATABASE CONNECTION
 *   go_home()								REDIRECTS TO HOME PAGE
 *   idx($arr, $index, $default=null)		RETURNS $arr[$idx]
 *   get_domain($url)						RETURNS DOMAIN OF URL
 *   no_magic_quotes($val)					REMOVES MAGIC QUOTES
 *   parse_http_args($http_params, $keys)	PARSES HTTP ARGUMENTS IN ARRAY
 */
...
function idx($arr, $index, $default=null) {
	if ($index === null || !is_array($arr)) {
		return $default;
	}
	$index = $index >= 0 ? $index : count($arr) + $index;
	
	if (array_key_exists($index, $arr)) { $result = $arr[$index];} else { $result = $default; }
	return $result;
}
